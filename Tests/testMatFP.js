...
function drawVec4(v, title = "") {
    addLine("Vector: " + title);

    var l = "<table><tr><td>";
    l += Math.floor(v[0] * 100) / 100;
    l += "</td><td>";
    l += Math.floor(v[1] * 100) / 100;
    l += "</td><td>";
    l += Math.floor(v[2] * 100) / 100;
    l += "</td><td>";
    l += Math.floor(v[3] * 100) / 100;
    l += "</td></tr></table>";
...
t.test1 = function() {
    let m1 = mat4.create();
    mat4.fromYRotation(m1, 30/180*3.14159);
    drawMatrix(m1, "m1");
    let m2 = mat4.create();
    //mat4.scale(m2, m1, [127, 127, 1, 1]);
    drawMatrix(m2, "m2");
...
t.test2 = function() {
    let m1 = mat4.create();
    mat4.perspective(m1, 60, 1, 127, 255);
    drawMatrix(m1);
    let m2 = mat4.create();
    mat4.translate(m2, m1, [80, 45, 0, 0]);
    drawMatrix(m2);

    let v2 = vec4.create();
    let v1 = vec4.fromValues(0.0, 0.0, 1.0, 1.0);
    drawVec4(v1, "v1 in");
    vec4.transformMat4(v2, v1, m2)
    drawVec4(v2, "v1 out");

    v1 = vec4.fromValues(0.0, 0.0, 192.0, 1.0);
    drawVec4(v1, "v1 in");
    vec4.transformMat4(v2, v1, m2)
    drawVec4(v2, "v1 out");

    v1 = vec4.fromValues(0.0, 0.0, 128.0, 1.0);
    drawVec4(v1, "v1 in");
    vec4.transformMat4(v2, v1, m2)
    drawVec4(v2, "v1 out");

    v1 = vec4.fromValues(0.0, 0.0, 254.0, 1.0);
    drawVec4(v1, "v1 in");
    vec4.transformMat4(v2, v1, m2)
    drawVec4(v2, "v1 out");
